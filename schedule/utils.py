...
def get_or_create_rule(day_char):
    ...
    if rule_string:
        ...
        rule, created = Rule.objects.get_or_create(
            name=rule_name,
            defaults={
                'description': f"Occurs every {rule_string}",
                'frequency': 'WEEKLY',
                'params': f"BYDAY={rule_string}"
            }
        )
        return rule
    return None

def create_course_events(student):
    from schedule.models import Calendar, Event, Occurrence
    # Get the student's calendar or update the existing one
    calendar_slug = f"{student.account.username}-calendar"
    calendar, created = Calendar.objects.get_or_create(
        slug=calendar_slug,
        defaults={
            'name': f"{student.account.username}'s Calendar",
        }
    )

    if not created:
        # If the calendar already exists, update its name
        calendar.name = f"{student.account.username}'s Calendar"
        calendar.save()

    # Delete existing events and occurrences for the student
    Event.objects.filter(calendar=calendar).delete()
    Occurrence.objects.filter(event__calendar=calendar).delete()
    ...
    for course in courses:
        # Parse days field
        days = [day.upper() for day in list(course.days)] if course.days else []
        days_2 = [day.upper() for day in list(course.days_2)] if course.days_2 else []
        all_days = set(days + days_2)

        for day_char in all_days:
            rule = get_or_create_rule(day_char)
            if rule:
                ...
                # Create occurrences for the event based on the rule
                event._create_occurence(start_time, end_time)
